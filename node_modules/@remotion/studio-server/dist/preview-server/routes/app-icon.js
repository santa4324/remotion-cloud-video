...
const github_desktop_artwork_1 = require("../../helpers/github-desktop-artwork");
...
const handleAppIcon = ({ request, response, pathname, }) => {
    ...
    const match = /^\/api\/app-icon\/(coding-agent|file-manager|git-client|terminal)\/([a-z0-9-]+)\.png$/.exec(pathname);
    if (!match) {
    ...
    const dataUrl = kind === 'git-client'
        ? id === 'github-desktop'
            ? github_desktop_artwork_1.githubDesktopArtworkDataUrl
            : null
        : kind === 'file-manager'
            ? id === 'finder'
