...
async function launchEditor({ colNumber, editor, fileName, lineNumber, vsCodeNewWindow, logLevel, }) {
    ...
    if (process.platform === 'win32' &&
        ...
        Log.error({ indent: false, logLevel }, 'Could not open ' + node_path_1.default.basename(fileName) + ' in the editor.');
        Log.error({ indent: false, logLevel });
        Log.error({ indent: false, logLevel }, 'When running on Windows, file names are checked against a whitelist ' +
    ...
    return new Promise((resolve, reject) => {
        ...
        _childProcess.on('exit', (errorCode) => {
            _childProcess = null;
            if (errorCode) {
                Log.error({ indent: false, logLevel }, `Process exited with code ${errorCode}`);
            }
        });
        _childProcess.on('error', (error) => {
            Log.error({ indent: false, logLevel }, 'Error opening file in editor', fileName, error.message);
