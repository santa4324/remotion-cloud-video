...
import * as recast24 from "recast";

// src/sequence-props.ts
import { LINEAR_KEYFRAME_EASING as LINEAR_KEYFRAME_EASING2 } from "@remotion/studio-shared/keyframe-easing-presets";
import { isKeyframeInterpolationFunction } from "@remotion/studio-shared/keyframe-interpolation-function";
import * as recast6 from "recast";
...
var getInterpolationMetadata = (interpolationFunction, callExpression, keyframeCount) => {
  ...
  const defaults = {
    easing: Array.from({ length: segments }, () => LINEAR_KEYFRAME_EASING2),
...
var getInterpolationKeyframes = (node, ast, videoConfigValues) => {
  ...
  if (callExpression.callee.type !== "Identifier" || !isKeyframeInterpolationFunction(callExpression.callee.name)) {
    return;
  }
    ...
    visitCallExpression(path) {
      ...
      for (const change of transformation.changes) {
        ...
        if (change.newValueIsUndefined) {
          ...
          parsed = parseAst(`a = ${stringifyDefaultProps({ props: JSON.parse(change.newValueSerialized), enumPaths: change.enumPaths })}`).program.body[0].expression.right;
        }
...
import * as recast9 from "recast";

// src/get-node-path-remappings.ts
import * as recast8 from "recast";
...
import * as recast10 from "recast";
...
import * as recast11 from "recast";
...
import * as recast15 from "recast";

// src/ensure-imports-and-frame-hook.ts
import * as recast12 from "recast";
...
import * as recast14 from "recast";

// src/update-nested-prop.ts
import { stringifyDefaultProps as stringifyDefaultProps2 } from "@remotion/studio-shared";
import * as recast13 from "recast";
var b7 = recast13.types.builders;
var parseValueExpression = (value) => {
  return parseAst(`a = ${stringifyDefaultProps2({ props: value, enumPaths: [] })}`).program.body[0].expression.right;
};
...
// src/recast-mods.ts
import * as recast16 from "recast";
...
import * as recast17 from "recast";
...
import * as recast18 from "recast";
...
import * as recast19 from "recast";
...
import * as recast20 from "recast";
...
import { stringifyDefaultProps as stringifyDefaultProps3 } from "@remotion/studio-shared";
import * as recast21 from "recast";
...
var updateDefaultProps = async ({
  ...
  const ast = parseAst(input);
  const stringified = stringifyDefaultProps3({
...
import {
  ...
  HOLD_KEYFRAME_EASING as HOLD_KEYFRAME_EASING2,
  isKeyframeInterpolationFunction as isKeyframeInterpolationFunction2,
  isSchemaFieldHoldOnly,
  isSchemaFieldKeyframable,
  LINEAR_KEYFRAME_EASING as LINEAR_KEYFRAME_EASING3
} from "@remotion/studio-shared";
import * as recast22 from "recast";
...
var getInterpolationExpression = (node, videoConfigValues) => {
  ...
  if (node.type !== "CallExpression" || node.callee.type !== "Identifier" || !isKeyframeInterpolationFunction2(node.callee.name)) {
    return null;
  }
...
var getInterpolationCalleeForValues = ({
  ...
  return b12.identifier(getKeyframeInterpolationFunction({
    schema,
    key,
    staticValue,
    newValue
...
var getKeyframeEasingArray2 = ({
  ...
  if (easingNode.type === "ArrayExpression") {
    ...
    while (easingArray.length < segmentCount) {
      easingArray.push(LINEAR_KEYFRAME_EASING3);
    }
    return easingArray;
  }
...
var normalizeEasingAfterRemovingKeyframe = ({
  ...
  while (easing.length < nextSegmentCount) {
    easing.push(LINEAR_KEYFRAME_EASING3);
  }
  return {
    extraArgs: getExtraArgsWithOptions({ extraArgs, options }),
...
var addKeyframe = ({
  ...
  if (existing) {
    const defaultEasing2 = isSchemaFieldHoldOnly({ schema, key }) ? HOLD_KEYFRAME_EASING2 : LINEAR_KEYFRAME_EASING3;
...
var applyKeyframeOperation = ({
  ...
  if (operation.type === "easing") {
    if (isSchemaFieldHoldOnly({ schema, key }) && operation.easing.type !== "step1") {
...
import * as recast23 from "recast";
