...
const getPackagesToUpgrade = ({ depsWithVersions, catalogEntries, targetVersion, extraPackageVersions, }) => {
    ...
    const remotionToUpgrade = list_of_remotion_packages_1.listOfRemotionPackages.filter((u) => allDeps.includes(u) || catalogPackages.has(u));
    const installedExtraPackages = Object.keys(extra_packages_1.EXTRA_PACKAGES).filter((pkg) => allDeps.includes(pkg) || catalogPackages.has(pkg));
...
const upgradeCommand = async ({ remotionRoot, packageManager, version, skipSkills, logLevel, args, }) => {
    ...
    const catalogEntries = workspaceRoot ? (0, catalog_utils_1.getCatalogEntries)(workspaceRoot) : {};
    const { normalPackages, catalogPackages } = (0, exports.getPackagesToUpgrade)({
        depsWithVersions,
        catalogEntries,
        targetVersion,
        extraPackageVersions,
    });
    if (normalPackages.some(({ pkg }) => pkg in extra_packages_1.EXTRA_PACKAGES) ||
        catalogPackages.some(({ pkg }) => pkg in extra_packages_1.EXTRA_PACKAGES)) {
        const installedExtraPackages = [
            ...normalPackages,
            ...catalogPackages,
        ].filter(({ pkg }) => pkg in extra_packages_1.EXTRA_PACKAGES);
