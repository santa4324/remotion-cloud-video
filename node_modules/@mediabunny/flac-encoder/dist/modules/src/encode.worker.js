...
const flac_1 = __importDefault(require("../build/flac"));
let module;
...
const ensureModule = async () => {
    if (!module) {
        ...
        modulePromise = (0, flac_1.default)();
...
const initEncoder = async (numberOfChannels, sampleRate, bitsPerSample) => {
    await ensureModule();
...
const encode = (ctx, audioData, numSamples) => {
    ...
    const handleCommand = async () => {
        try {
            ...
            switch (command.type) {
                case 'init':
                    {
                        const { ctx, header } = await initEncoder(command.data.numberOfChannels, command.data.sampleRate, command.data.bitsPerSample);
                ...
                case 'encode':
                    {
                        const packets = encode(command.data.ctx, command.data.audioData, command.data.numSamples);
